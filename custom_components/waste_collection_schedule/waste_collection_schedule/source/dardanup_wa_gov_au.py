...
# Streets that appear in paragraph text as "Suburb - Thursday" patterns
_THURSDAY_SUBURBS = {"burekup", "dardanup townsite"}
...
class Source:
    ...
    def fetch(self) -> list[Collection]:
        if self._collection_day:
            # Direct day override — no scraping needed
    ...
    def _lookup_street_day(self) -> int:
        ...
        # Inner accordions hold day headings (h2) + street paragraph lists (p)
        for accordion in soup.select("div.accordion div.accordion"):
            ...
            for child in accordion.children:
                ...
                if child.name == "h2":
                    ...
                    # "Burekup - Thursday" / "Dardanup Townsite - Thursday" pattern
                    # (these appear under a non-day heading so current_day is None)
                    ...
                    # Normal comma-separated street list (only when under a day heading)
